...
function buildSystemPrompt() {
  return [
    ...
    'Your task is to read the user input and propose one or more entries for every distinct thing worth saving.',
    ...
    'If the input is only describing a Finance expense, do not create personal memory proposals unless the user also describes a real experience, opinion, recommendation, visited place, or item they want to remember.',
    'Do not create personal memory proposals merely because an expense mentions background context, destination, occasion, merchant type, product type, payment reason, or reimbursement context.',
    'For expense-only inputs, treat purchased goods or services as transaction details, not personal memories, unless the user expresses an experience, opinion, recommendation, preference, or explicit desire to remember that item/place.',
    'Only create restaurant or food proposals when the user explicitly describes eating, drinking, visiting a restaurant/bar/cafe, or having an opinion about the place or meal.',
    'If the user mentions multiple distinct items for the same section, create multiple proposals for that section.',
    'Propose for every relevant section supported by the provided context.',
